...
- 本ドキュメントは、React Router v7プロジェクトの実装ルールを定義する。
- 現在はSPA構成（`ssr: false`）で運用中。`loader` はサーバーではなくクライアント上で実行される。
...
```
app/
  config/           # 環境変数やアプリケーション全体の設定（権限、ルーティングなど）
  routes/           # ルートモジュール（HTTP境界）
  features/         # 機能単位でスコープされたモジュール（UI、フック、モデルなど）
    <feature-name>/ # 各機能のディレクトリ（例: auth, homeroom）
      components/   # 機能に特化したUIコンポーネント
      model/        # その機能固有のドメインモデル（エンティティ、値オブジェクトなど）
      pages/        # ルートレベルのページコンポーネント（ルートで構成され、再利用はしない）
  domain/           # （オプション）複数の機能で共有される純粋なドメインロジックが必要な場合
  components/       # アプリケーション全体で共有するプレゼンテーションコンポーネント（uiなど）
  hooks/            # クライアント側のアプリケーションロジック
  usecases/         # サーバー側のアプリケーションロジック（オプション。必要に応じて作成）
  service/          # 外部I/Oの抽象化と正規化
  lib/              # 技術的なユーティリティのみ（ビジネスロジックは不可）
  types/            # アプリケーション全体で共有する型定義（UI型など）
...
### 設定・環境変数の配置 (Configuration & Env)

環境変数のパースおよびアプリケーション全体の設定（権限やルーティングの設定など）は `config/` 配下に配置しなければならない。
シークレットや環境変数は、必要に応じて `config/env.ts` などの一元化されたモジュールを通じてのみアクセスすること。
...
機能単位でスコープされたコンポーネント（その機能固有の部品）
→ `features/<feature-name>/components/`

機能をまたいで再利用可能なコンポーネント（共通UIなど）
→ `components/` 配下（例: `components/ui/`）
...
エンティティや値オブジェクトなどのドメインロジックは、原則として各機能の `features/<feature-name>/model/` ディレクトリに配置する。
ただし、複数の機能で横断的に共有される純粋なドメインルールが存在する場合は、ルートの `domain/` ディレクトリを作成してそこに配置してもよい。
...
- 汎用的なフォーマットヘルパー
- 汎用的な関数型ユーティリティ
- 環境変数アクセスヘルパー（クライアント側で安全なもの、または一元管理されるものは `config/` 配下に置くこと）
...
- **ルートモジュールレベルの `ErrorBoundary`**：そのルート画面の障害を処理する（例：`app/routes/main/sports.tsx` に配置）
- **アプリルートレベルの `ErrorBoundary`**：アプリ全体のグローバルな障害を処理する（例：`app/root.tsx` に配置）
